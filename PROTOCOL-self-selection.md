...
- **Agreement redefinition (Stage-2 prerequisite).** Domain-level convergence vs path-embedding cosine vs seeding walkers from a shared frontier — which best captures "the walkers agree" without the dispersed-seed structural zero? Prototype against logged walk sessions before committing.
